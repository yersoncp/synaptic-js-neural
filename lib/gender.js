...
var _window$synaptic = window.synaptic,
    Architect = _window$synaptic.Architect,
    Trainer = _window$synaptic.Trainer;
var INPUT_LENGTH = 7;
var myNetwork = new Architect.Perceptron(INPUT_LENGTH, 6, 1);

function convertNameToInput(name) {
  name = name.toLowerCase();
  if (name.length > INPUT_LENGTH) name = name.substring(INPUT_LENGTH);
  ...
  return characters.map(function (c) {
    return c == " " ? 0 : c.charCodeAt(0) / 1000;
  });
}
